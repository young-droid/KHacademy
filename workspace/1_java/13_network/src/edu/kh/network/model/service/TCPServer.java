...
public class TCPServer {
	...
	/*
	 * Server : 서비스를 제공하는 컴퓨터 또는 프로그램 Client : 서버에 서비스를 요청하여 사용하는 프로그램 또는 컴퓨터
	 * 
	 * TCP Socket 프로그래밍
	 * 
	 * TCP : 데이터 전달의 신뢰성을 최대한 보장하기 위한 방식(연결 지향형 통신) 순차적으로 데이터를 전달하고 확인 및 오류 시 재전송
	 * 
	 * Socket : 프로세스의 통신에 사용되는 양 끝단 서버와 클라이언트가 통신하기 위한 매개체
	 * 
	 * ServerSocket : 서버용 소켓 - Port와 연결되어 외부 요청을 기다리는 역할 -> 클라이언트 요청이 올 경우 이를
	 * 수락(accept)하고 클라이언트가 사용할 수 있는 소켓을 생성 --> 서버 소켓과 클라이언트 소켓이 연결되어 데이터 통신이 가능해짐
	 * 
	 * 
	 */
