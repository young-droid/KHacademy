...
document.getElementById("userPw2").addEventListener("keyup", function () {
    ...
    */
    ...
    if (userPw.value != "") { // userPw 입력 시
        if (userPw.value === this.value) {
            // - 비밀번호가 일치할 경우
            // "비밀번호" 입력창 오른쪽에 "비밀번호 일치" 글자를 초록색으로 출력.
            userPw.parentNode.nextElementSibling.classList.add("confirm");
            userPw.parentNode.nextElementSibling.classList.remove("error");
            userPw.parentNode.nextElementSibling.innerText = "비밀번호 일치";
        } else {
            // - 비밀번호가 일치하지 않을 경우
            // "비밀번호" 입력창 오른쪽에 "비밀번호가 불일치" 글자를 빨간색으로 출력
            userPw.parentNode.nextElementSibling.classList.add("error");
            userPw.parentNode.nextElementSibling.classList.remove("confirm");
            userPw.parentNode.nextElementSibling.innerText = "비밀번호가 불일치";
        }
    }

    console.log(userPw.nextElementSibling);
