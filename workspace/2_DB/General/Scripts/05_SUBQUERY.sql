...
-- 	  상관 커리는 메인쿼리가 사용하는 테이블값을 서브쿼리가 이용해서 결과를 만듦
...
SELECT EMP_NAME , DEPT_CODE , HIRE_DATE , ENT_YN FROM EMPLOYEE ;
...
-- EXISTS : 서브쿼리에 해당 하는 행이 1개라도 존재하면 조회결과에 포함 
...
SELECT MAIN.EMP_ID , MAIN.EMP_NAME , NVL(MAIN.MANAGER_ID , '관리자 없음'), NVL((SELECT EMP_NAME FROM EMPLOYEE SUB 
														WHERE (SUB.EMP_ID = MAIN.MANAGER_ID)),'-')
FROM EMPLOYEE MAIN;
...
FROM (SELECT DEPT_CODE , FLOOR(AVG(SALARY)) "평균 급여"
	  FROM EMPLOYEE 
	  GROUP BY DEPT_CODE 
...
--				 EX) 공동 1위가 2명이면, 다음 순서는 2위가 아니라 3위
...
WITH "2000년도" AS (SELECT DEPT_CODE , JOB_CODE
FROM EMPLOYEE 
...
-- 1) 퇴사한 직원을 제외하고 부서별 입사일이 가장 빠른 직원
...
-- 직급별 나이가 가장 어린 직원 테이블
SELECT MIN(FLOOR((SYSDATE-TO_DATE(SUBSTR(EMP_NO,1,2),'RRRR'))/365)) "나이"
FROM EMPLOYEE SUB;
WHERE (SUB.JOB_CODE = MAIN.JOB_CODE);


SELECT EMP_ID , EMP_NAME , JOB_NAME , FLOOR(MONTHS_BETWEEN(SYSDATE, TO_DATE(SUBSTR(EMP_NO,1,6), 'RRMMDD'))/12) "나이" , TO_CHAR(SALARY*(1+NVL(BONUS,0))*12, 'L999,999,999') AS "연봉"
FROM EMPLOYEE MAIN
JOIN JOB ON (MAIN.JOB_CODE = JOB.JOB_CODE)
WHERE FLOOR((SYSDATE-TO_DATE(SUBSTR(EMP_NO,1,2),'RRRR'))/365) = 
