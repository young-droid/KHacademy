...
--> 각자 이니셜 계정에 VIEW 생성 권한을 부여 (관리자 계정으로 접속)
GRANT CREATE VIEW TO KH_CYJ;
...
SELECT * FROM V_EMP WHERE 직급명 = '사원';
...
-- ORA-01400: NULL을 ("KH_CYJ"."DEPT_COPY2"."DEPT_TITLE") 안에 삽입할 수 없습니다.
...
-- 227 부터 5씩 증가하는 시퀀스 생성
CREATE SEQUENCE SEQ_EMP_ID
  START WITH 227 -- 처음 발생시킬 시작값 지정, 생략하면 자동 1이 기본
  INCREMENT BY 5; -- 다음 값에 대한 증가치, 생략하면 자동 1이 기본
...
-- NEXTVAL 호출1 
SELECT SEQ_EMP_ID.NEXTVAL FROM DUAL;  -- 최초 호출 == START WITH 값 조회된다 
-- 227
...
- 인덱스 내부 구조는 B*(바이너리 BINARY) 트리 형식으로 되어있음.
...
CREATE INDEX IDX_EMP_NAME ON EMPLOYEE_COPY4(EMP_NAME);
