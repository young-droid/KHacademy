...
-- DEP_CODE가 D9 또는 D6이고
-- 고용일이 90/01/01 ~ 99/12/01 이고
-- 급여가 270만 이상인 사원의 이름, 이메일, 부서코드, 입사일, 급여를 조회

SELECT EMP_NAME, EMAIL, DEPT_CODE, HIRE_DATE, SALARY 
FROM EMPLOYEE 
WHERE EMAIL LIKE '____$_%' ESCAPE '$'
AND (DEPT_CODE = 'D9' OR DEPT_CODE = 'D6')
AND HIRE_DATE BETWEEN '1990/01/01' AND '1999/12/01' 
AND SALARY >= 2700000;
...
SELECT EMP_ID, EMP_NAME
FROM EMPLOYEE 
WHERE EMP_ID IN (200, 205, 210); /* 홑따옴표 있어도 없어도 된다*/
...
-- 연결 연산자( || )
-- 여러 값을 하나으 ㅣ컬럼 값으로 연결하는 연산자
-- (자바의 문자열 이어쓰기 효과)

SELECT EMP_NAME || '의 급여는 ' || SALARY || '원 입니다.' AS "RESULT"
FROM EMPLOYEE ;
 ...
 * - DB에서 NULL : 컬럼 값이 없다.
 * 
 * 1) IS NULLL : 컬럼 값이 NULL인 경우 조회
 * 2) IS NOT NULL : 컬럼 값이 NULL이 아닌 경우 조히
 * 
 * [작성법]
 * 컬럼값 IS [NOT] NULL
 */
...
FROM EMPLOYEE 
ORDER BY 2 DESC ; --> 2번째 컬럼을 내림차순

-- 연봉 내림 차순
SELECT EMP_NAME, SALARY * 12 연봉
FROM EMPLOYEE 
--WHERE 연봉 >= 50000000 --> WHERE절 해석 순서가 별칭 해석 전이기 때문에 오류 발생
WHERE SALARY*12 >= 50000000
--ORDER BY 연봉 DESC; --> 별칭 사용
ORDER BY SALARY*12 DESC; --> 컬럼명 사용
ORDER BY 2 DESC; --> 컬럼 순서 사용
...
ORDER BY BONUS NULLS FIRST; -- NULLS 를 먼저 출력한 뒤 기본값 오름차순 정렬
...
FROM EMPLOYEE 
ORDER BY DEPT_CODE , SALARY DESC; --> 정렬 중첩
