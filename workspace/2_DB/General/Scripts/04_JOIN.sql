...
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID); 
--> ANSI 에서는 연결에 사용할 컬럼명이 다른 경우 ON()을 사용
...
-- 		NULL	= (D1~D9)(+)
-- DEPT_CODE의 값(NULL)이
-- DEPT_ID의 값(D1~D9)과 같이 않아도 추가해라


-- 1) RIGHT [OUTER] JOIN : 합치기에 사용한 두 테이블 중 **오른편에** 기술된 테이블의 
-- 						  컬럼 수를 기준으로 JOIN
...
-- 3. 교차 조인(CROSS JOIN == CARTESIAN PRODUCTION)
...
-- ANSI 표준
SELECT EMP.EMP_ID , EMP.EMP_NAME , NVL(EMP.MANAGER_ID , '없음'), MG.EMP_NAME "사수 이름"
FROM EMPLOYEE EMP
...
WHERE DEPT_ID = DEPT_CODE -- EMPLOYEE, DEPARMENT 조인
...
FROM EMPLOYEE 
WHERE SUBSTR(EMP_NO,8,1) = '2' AND EMP_NO LIKE '7%'  AND EMP_NAME LIKE '전%';
...
--WHERE DEPT_ID IS NOT NULL;
	--> 이미 위 JOIN 을 하며 NULL값을 필터링 했기 때문에 굳이 WHERE 을 작성할 필요 없음 

-- ****** 진행중
-- 6. 급여등급별 최소급여(MIN_SAL)를 초과해서 받는 직원들의
-- 사원명, 직급명, 급여, 연봉(보너스포함)을 조회하시오.
-- 연봉에 보너스포인트를 적용하시오.
SELECT SAL_LEVEL , SALARY 
FROM EMPLOYEE ;

SELECT SAL_LEVEL , MIN_SAL 
FROM SAL_GRADE ;

SELECT EMP_NAME , JOB_NAME , JOB_CODE , SALARY , SALARY * (1+NVL(BONUS,0)) * 12 "연봉"
FROM EMPLOYEE 
JOIN JOB USING (JOB_CODE)
...
SELECT E2.EMP_NAME, E.DEPT_CODE , E.EMP_NAME 동료이름
FROM EMPLOYEE E
...
WHERE EMPLOYEE.JOB_CODE IN ('J4', 'J7');
...
--> 자연 조인은 테이블간에 동일한 형식을 갖는 공통 컬럼이 반드시 하나만 존재해야 함! 
